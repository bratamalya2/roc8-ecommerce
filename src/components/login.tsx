...
function Login() {
    ...
    return <section className="w-[50%] h-[691px] border border-[#C1C1C1] rounded-[20px] flex flex-col items-center py-10">
        <p className="font-semibold text-lg sm:text-3xl">Login</p>
        <p className="mt-14 text-2xl font-medium">Welcome back to ECOMMERCE</p>
        <p className="mt-5 mb-14">The next gen business marketplace</p>
        <div className="my-5 w-[80%] mx-auto">
            <label>Email</label>
            <br />
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Enter your email" className={`border border-[#C1C1C1] rounded-[6px] h-[48px] px-2 w-full`} />
        </div>
        <div className="my-5 w-[80%] mx-auto relative">
            <label>Password</label>
            <br />
            <input id="login-password" type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="Enter your password" className={`border border-[#C1C1C1] rounded-[6px] h-[48px] px-2 w-full`} />
            <div className="absolute right-2 bottom-3 underline hover:cursor-pointer" onClick={togglePasswordVisibility}>{passwordVisibilityText}</div>
        </div>
        ...
        <div className="text-sm sm:text-base">Don't have an Account? <Link href="/"><span className="uppercase font-medium hover:cursor-pointer">Sign up</span></Link></div>
    </section>
}
